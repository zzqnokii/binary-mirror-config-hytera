...
exports.MirrorConfig = class MirrorConfig {
  constructor(options = {}) {
    ...
    this.ungzipDir = options.ungzipDir;
    ...
    if (binaryMirror) {
      ...
      if (installScripts && !binaryMirror.replaceHostFiles) {
        ...
        if (/prebuild --install|prebuild --download|node-pre-gyp install/.test(installScripts) ||
          ...
          if (/node-pre-gyp install/.test(installScripts)) {
            const versioningFile = path.join(this.ungzipDir, 'node_modules/node-pre-gyp/lib/util/versioning.js');
            try {
        ...
        for (const replaceHostFile of replaceHostFiles) {
          const replaceHostFilePath = path.join(this.ungzipDir, replaceHostFile);
      ...
      if (pkgName === 'cypress') {
        ...
        if (targetPlatform) {
          ...
          const downloadFile = path.join(this.ungzipDir, 'lib/tasks/download.js');
        ...
        const indexFilepath = path.join(this.ungzipDir, 'bin/install');
